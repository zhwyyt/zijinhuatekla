...
- 零件特征（最终版）：`outputs/selection-main-plus-features-20260922/recognition-main-plus-features-geometry-v7.xlsx`
- 构件复杂度（最终版）：`outputs/member-complexity-20260922/member-complexity-geometry-v9.xlsx`
...
- 零件特征：4044 行，失败 0。
- 构件复杂度：289 行，失败 0。
- 零件特征汇总（v7）：剖口 332、倒角 1085、割孔 360、切割 1025、洞口 313、螺栓孔 1258、焊接垫板 141。
...
- 剖口修正：`T3-H-528` 的唯一 `CUT_PLANE` 记录被导出器标为 `isBevel=true`，但全部剖口尺寸为 0，几何事实是垂直切割。规则改为零尺寸明细不计剖口；全模型共排除 238 条此类记录，涉及 171 个零件。修正后 `T3-H-528` 为剖口=否、切割=是。
- 埋件例外：构件号包含 `MJ` 时直接输出埋件，不再执行主材几何判定；零件表不标主材，但切割、洞口、螺栓孔、剖口等零件特征照常计算。当前 289 装配选择集没有 `MJ` 构件，因此已发布汇总数值不变。
...
- 追加回归：`CUT_PLANE` 明细所有剖口尺寸为 0 时不是剖口。
- 追加回归：`MJ` 构件主材类型=埋件、主材形态=不适用；零件表构件类型=埋件、主材=否，切割和螺栓孔仍为是。
