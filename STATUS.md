...
2026-09-20：`0.3.4-single-pass` 已用当前选择集实测。289 根装配约 11.2 min；剖口与 0.3.3 dump 重叠 **0 差异**；识别 4044 行、方块且倒角/洞口=0。关采样后 Box 叠层过宽，把 `6GL` 的 H 主板标成 BOX主壁板（416 行主材差异）。已改为双证据不覆盖，需重导才进 Excel。记录见 `docs/verification/2026-09-18-exporter-index-once.md`。
